...
export async function POST(request: NextRequest) {
  try {
    ...
    // 调用后端API添加次数
    const response = await fetch(`${apiUrl}/v1/add_count_credits?paid_key=${encodeURIComponent(paid_key)}&amount=${amount}`, {
      method: "POST",
      headers: {
    ...
    console.error("Error adding count credits:", error)
